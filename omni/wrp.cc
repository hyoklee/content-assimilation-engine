...
// Forward declaration for put/get functionality
...
#ifdef USE_HERMES
int put_hermes_tags(hermes::Context *ctx, hermes::Bucket *bkt,
                    std::string tags) {
...
int run_lambda(std::string lambda, std::string name, std::string dest) {
  ...
  try {
    ...
    Poco::ProcessHandle ph =
      Poco::Process::launch(command, args, 0, &outPipe, &errPipe);
    ...
    std::cout << "STDERR:\n" << stderr_output;
    std::cout << "-----------------------------------\n";
    std::cout << "Lambda script exited with status: " << exitCode << std::endl;
...
int write_s3(std::string dest, char *ptr) {
  ...
  if (ptr == NULL) {
    ...
    std::shared_ptr<Aws::FStream> inputData = Aws::MakeShared<Aws::FStream>(
									    "SampleAllocationTag", filePath.c_str(),
    ...
    while (redirected && redirectCount <= maxRedirects) {
      ...
      if (startByte >= 0) {
        ...
        if (endByte == -1) { // Request bytes from startByte to end of file
          rangeHeaderValue = "bytes=" + std::to_string(startByte) + "-";
      ...
      if (status == Poco::Net::HTTPResponse::HTTP_MOVED_PERMANENTLY ||
        ...
        if (os.is_open()) {
          Poco::StreamCopier::copyStream(rs, os);
          os.close();
          std::cout << "File downloaded successfully to: " << outputFileName
                    << std::endl;
          return 0;
        } else {
          std::cerr << "Error: Could not open file for writing: "
...
int read_omni(std::string input_file) {
...
#endif
            if (read_exact_bytes_from_offset(path.c_str(), offset, nbyte, ptr) == 0) {
#ifdef DEBUG
              std::cout << "buffer=" << ptr << std::endl;
#endif
              put_data(name, tags, path, ptr, nbyte);
            } else {
              return -1;
            }
          }
  ...
  if (path.find("hdf5://") != path.npos) {
      ...
      if (buffer && buffer_size > 0) {
        // Use put_data to write the buffer
        int result = put_data(
            dc.name,                     // name
            tags,                        // tags
            "",				 // path (empty for now)
        ...
        if (result != 0) {
          std::cerr << "Error: Failed to write buffer using put_data()" << std::endl;
        } else {
          std::cout << "Successfully wrote buffer using put_data()" << std::endl;
        }
        
        // Free the allocated buffer
        delete[] buffer;
      } else {
        std::cerr << "Error: Failed to read dataset or empty dataset" << std::endl;
      }
  }
#endif
  ...
  if (!dest.empty()) {
	...
	Poco::File file(name);
	if (!file.exists()) {
	  throw Poco::FileNotFoundException("Error: buffer '" + name + "' not found");
	}
	Poco::SharedMemory shm_r(file, Poco::SharedMemory::AM_READ);
	std::cout << "read '" << shm_r.begin() << "' from '" << name << "' buffer." << std::endl;
	...
	std::cerr << "Error: lambda failed to generate '" << dest << "'" << std::endl;
      }
    } catch (Poco::Exception &e) {
...
int main(int argc, char *argv[])
{
  ...
  if (command == "put") {
    ...
    // Not an HDF5 file or no HDF5 source found, use the default processor
