...
bool parse_globus_uri(const std::string &uri, std::string &endpoint_id, std::string &path) {
    ...
    if (endpoint_id.empty()) {
        std::cerr << "Error: Empty endpoint ID in Globus URI: " << uri << std::endl;
        return false;
    }
...
bool transfer_globus_file(
    ...
    extern bool transfer_globus_file_impl(
        const std::string& source_uri,
        const std::string& dest_uri,
        const std::string& access_token,
        const std::string& transfer_label
    ...
    return transfer_globus_file_impl(source_uri, dest_uri, access_token, transfer_label);
}
